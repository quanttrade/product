...
df_return = df_return[1:]
print(df_return)
id_max = df_return.idxmax(axis=1)
print(id_max)
x = len(df_return)
ret1 = []
for i in range(x-1):
    ...
    col =id_max.iloc[i]
    print(col)
    ret1.append((df_return.loc[i+2,[col]].values[0]))
print(ret1)
ret_2 = np.array(ret1)-np.array(ret)
plt.plot(np.cumsum(ret1))
plt.plot(np.cumsum(ret_2))
