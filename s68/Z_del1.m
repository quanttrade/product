...
tmp0 = {'as51','hsce','hsi','topix','twse','kosdaq', 'kospi',... 
    'msci', 'ndx', 'nifty', 'nky', 'RTY', 'set50', 'sx5e','ukx', 'xin9i'};
tmp1 = {'AS51','HSCEI','HSI','TPX','TWSE','KOSDAQ','KOSPI2','TAMSCI','NDX',...
    'NIFTY','NKY','RTY','SET50','SX5E','UKX','XIN9I'};
%index0_pool =tmp0([1:13,16]);
index0_pool = tmp0(14:15);
index0_pool2 =containers.Map(tmp0, tmp1);
    ...
    for w_sel = 1:2
        ...
        if isempty(sub_sub_pos)
            ...
            if ~isempty(tmp)
                ...
                tmp = tmp{end}(:,end);
                tmp = tmp(2);
                [~,~,ia] = intersect(tmp,month_cut_date2(i_pool));
                i0_0 = max(ia);
                i0_0 = i0_0+1;
            else
                i0_0 = 1;
