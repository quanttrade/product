function [v,v_str,sta_val] = curve_static_month(y,print_sel)
if nargin < 2
    print_sel=0;
end
wid_month = 12;
...
%1年化收益率
v_str{1} = '年化收益率';
v(1) = (y(end)/y(1))^(wid_month/length(y))-1;
sta_val.nh = v(1);
%2最大回撤
v_str{2} = '最大回撤';
...
%3-5持有一年最小\最大\平均平均收益
if length(y)>=wid_month
    v1 = y(wid_month:end)./y(1:end-wid_month+1)-1;
else
    v1 = nan;
end
v_str{3} = '持有1年最低收益';
...
%8收益波动率
    %std(每日收益率)*sqrt(252)
v_str{8} = '收益波动率';
v(8) = std(y(2:end)./y(1:end-1)-1)*sqrt(wid_month);%年化标准差
...
%vv = 1.03;
vv = 1;
v(9) = ((mean(temp)-(exp(log(vv)/wid_month)-1)))/(std(temp))*sqrt(wid_month);
sta_val.sharp = v(9);
%10最长新高间隔
v_str{10} = '最长新高间隔';
temp = getdrawdown(y);
