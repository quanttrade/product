...
key_str2 = containers.Map({'000905','000300'},{'cis500','csi300'});
index0_pool = {'000905','000905','000300'};
symbol_pool_all = {'000905','000300','000300'};
symbol_pool_info = cell(size(index0_pool));
for i = 1:length(symbol_pool_info)
    symbol_pool_info{i} = sprintf('%s-%s',key_str2(index0_pool{i}),key_str2(symbol_pool_all{i}));
end
T_symbol_pool_all = length(symbol_pool_all);
if write_doc_sel
    obj_w = wordcom(fullfile(pwd,'S68P1-CSI.doc'));
end
sta_re = [];
for w_sel = 1:2
    for pool_id = 3%1:T_symbol_pool_all
        ...
        parfor i0 = 1:T2
            ...
            [wp,sub_symbol_pool] = sparseIndexTracking_CSI500(sub_tref,sub_y,window,index_pool,w_sel,key_str2(index0));
            [~,ia] = sort(wp,'descend');
            ia = ia(1:40);
            wp = wp(ia);
            %wp(ia(41:end)) = 0;
            ...
            [sub_r,sub_t_u,sub_symbol_u] = get_interchgPct_s68p2(sub_t1,sub_t2,sub_symbol_pool);
