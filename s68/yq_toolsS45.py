...
db_datapro = 'data_pro'
#eng_str='mysql+pymysql://%s:%s@localhost:%d/%s?charset=utf8' % (user_name,pass_wd,port,db_name)
eng_str='mysql+pymysql://%s:%s@localhost:%d/%s?charset=utf8' % (user_name,pass_wd,port,db_datapro)
eg_datapro = create_engine(eng_str)
...
#开始时间或终止时间
def get_table_date(tn,eg,key,order=1):
