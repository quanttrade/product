...
t2 = '2020-06-18';
symbol_pool_all = {'SPX'};
symbol_pool_info = {'SP500'};
T_symbol_pool_all = length(symbol_pool_all);
if write_doc_sel
    obj_w = wordcom(fullfile(pwd,'SP500结果.doc'));
end
sta_re = [];
for pool_id = 1:T_symbol_pool_all
    ...
    sql_str_index = ['SELECT tradeDate,closeIndex FROM S50.index_sina where ticker = ''%s'' ',...
    ...
    parfor i0 = 1:T2
        ...
        [wp,sub_symbol_pool] = sparseIndexTracking_US(sub_tref,sub_y,window,index_pool,w_sel);
        [~,ia] = sort(wp,'descend');
        ia = ia(1:40);
        %wp(ia(41:end)) = 0;
        wp = wp(ia);
        ...
        [sub_r,sub_t_u,sub_symbol_u] = get_interchgPct_US(sub_t1,sub_t2,sub_symbol_pool);
