%{
The M-files for the full factorization algorithm smooth and the truncated algorithm tsmooth are given below. The
inputs to smooth are the row vector of measurements and the smoothing parameter ?. The outputs are the row
vector of estimates and the GCV score. The inputs to tsmooth are the row vector of measurements, the smooth-
ing parameter ?, and the error exponent J. The outputs are the row vector of estimates and the GCV
score.
%}
function[x,score] = smooth_s30(y,sig)
