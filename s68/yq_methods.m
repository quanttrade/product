...
classdef yq_methods< handle
    methods
        %获取polygon前复权数据
        function X=get_polygon_chg(obj,ticker,t0,tt)
            c = obj.get_us_adj_coef(ticker);
            
            x = obj.get_us_daytick(ticker,t0,tt);
            if isempty(x)
                X = [];
            else
                %x复权系数
                if isempty(c)
                    ...
                    c(:,1) = cellstr(datestr(datenum(c(:,1))-1,'yyyy-mm-dd'));
                    t1=  datenum(x(:,3));
                    t2 = datenum(c(:,1));

                    v = sort(unique([t1;t2]));
                    [~,ia1,ib] = intersect(v,t1);
                    n=size(x,2);
                    m = length(v);
                    X = cell(m,n+1);
                    X(:) = {nan};
                    X(ia1,1:n) = x(ib,:);
                    [~,ia,ib] = intersect(v,t2);
                    X(ia,n+1) = c(ib,end);
                    %先filling
                    v1 = cell2mat(X(:,end));
                    if isnan(v1(end))
                    ...
                    v1 = fillmissing(v1,'next');
                    %1ticker,2t,3tradeDate,4openPrice,5highPrice,6lowPrice,7closePrice,trunoverVol,vw
                    X(:,end) = num2cell(v1);
                    
                    X = X(ia1,:);
                    v1 = v1(ia1,:);
                    
                    X(:,4:7) = num2cell(cell2mat(X(:,4:7)).*repmat(v1,1,4));
                end
            end
        end
    end
    methods(Static)
        ...
        function tref_month = get_month_end_inter(t0,tt)
            sql_str = 'select endDate from yuqerdata.yq_index_month where symbol = ''000001'' and endDate>="%s" and endDate<="%s" order by endDate';
            tref_month = fetchmysql(sprintf(sql_str,t0,tt),2);
        end
        %获取周末日期
        ...
        %获取S68指数成分股
        function s = get_index_com_s68(index)
            tn = 'data_pro.main_index_s68';
            index_p = containers.Map({'as51','hsce','hsi','topix','twse'},...
                {'AS51','HSCEI','HSI','TPX','TWSE'});
            sql_tmp = 'select distinct(ticker) from %s where index_id = "%s" and ticker != "%s"';
            s = fetchmysql(sprintf(sql_tmp,tn,index,index_p(index)),2);
        end
        %获取sp500成分股
        function s= get_index_com_sp500(t0)
            if nargin <1
                t0 = datestr(now,'yyyy-mm-dd');
            end
            sql_str = 'select tradeDate,tickers from data_pro.sp500_com where tradeDate<="%s" order by tradeDate desc limit 1';
            s=fetchmysql(sprintf(sql_str,t0),2);
            if ~isempty(s)
                s = unique(strsplit(s{2},','));
            end
        end
        %获取polygon数据
        function x = get_us_daytick(ticker,t0,tt)
            if nargin < 1
                ticker='AAPL';
            end
            if nargin < 2
            ...
            sql_str = ['select * from polygon.usastock_day where ticker = "%s" ',... 
                'and tradeDate>="%s" and tradeDate<="%s" order by tradeDate '];
            x = fetchmysql(sprintf(sql_str,ticker,t0,tt),2);
        end
        %获取系数
        function x= get_us_adj_coef(ticker)
            if nargin < 1
                ticker= "AAPL";
            end
            sql_str = 'select exDate as tradeDate,ratio from polygon.p_split where ticker = "%s" order by tradeDate desc';
