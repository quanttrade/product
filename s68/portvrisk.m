...
%  $Revision: 1.7.2.2 $   $ Date: 1998/05/11 13:45:34 $
...
if (nargin < 2)
   error('finance:portvrisk:missingInputs','You must enter PortReturn and PortRisk');
end
...
if(any(size(PortReturn) ~= size(PortRisk)))
   error('finance:portvrisk:mismatchReturnRisk','PortReturn and PortRisk must be of the same length');
end
...
if ( any( Size ~= [1 1] ) )
  ...
  if isempty(OldSize)
    ...
    if ( any( Size ~= OldSize ) )
      error('finance:portvrisk:mismatchInputs','Arguments must be scalars or consistent vectors');
    end
  end  
end
