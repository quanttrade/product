...
%使用polygon数据，爬取的sp500成分股权重数据执行
function [wp,sub_symbol_pool,OK] = sparseIndexTracking_US_polygon(tref,r,window,index_pool,id)
    if nargin < 5
        id = 1;
    end
    pn0=pwd;
    ...
    sub_symbol_pool = yq_methods.get_index_com_sp500(tref{end})';
    ...
    if exist(fn_re,'file')
        ...
        sub_t1 = tref{1};
        sub_t2 = tref{end};
        [sub_r,sub_t_u,sub_symbol_u] = get_interchgPct_us_polygon(sub_t1,sub_t2,sub_symbol_pool);
