...
mk = markov_switch(up_switch=[0.005, 0.005, 0.99],
                   ...
                   flat_switch=[0.005, 0.99, 0.005], noisy_level=0.008)
ou = piece_ou_process()
...
best_score = 100
for i in range(1000):
    mk_x, _, y = generate_train_data(size=16, random_state=i)
...
hs_pred = best_model(hs_ten).argmax(dim=1).numpy() - 1

# %%
signal_up = hs_pred > 0
plt.plot((s[1:] + 1).cumprod())
plt.plot((s[1:] * signal_up[:-1] + 1).cumprod())
plt.show()
