...
y0['rf'] =y0.r-y0.r1
y0.rename(columns = dict(zip(['r1', 'r', 'rf'],['空','多','多空对冲'])),inplace=True)
(1+y0).cumprod().plot(rot=30)
...
y1['v'] = y1.r-y1.r0
y1.rename(columns = dict(zip(['r', 'r0', 'v'],['策略多','指数','策略对冲指数'])),inplace=True)
(1+y1).cumprod().plot(rot = 30)