...
for t in tqdm(tref2):
    ...
    b = model.coef_  # 回归系数
    tmp = pd.DataFrame({t:b.tolist()+[a]})
    B.append(tmp.T)
B = pd.concat(B)

#从2017-05-19选股，持有2周，然后下个周末再次选股
_,tref_w,_,_ = get_tref('2017-05-19','3033-01-01')
#
tref_w = tref_w[::2]
BF = B.rolling(9).mean()
r = []
...
y0['rf'] =y0.r-y0.r1
y0_tmp = y0.rename(columns = dict(zip(['r', 'r1', 'rf'],['多','空','多空对冲'])))
...
y1= y.merge(r0,left_index=True,right_index=True)
y1['v'] = y1.r-y1.r0
(1+y1).cumprod().plot()

y1_tmp=y1.rename(columns = dict(zip(['r', 'r0', 'v'],['策略多','指数','策略对冲指数'])))
