...
if os.path.exists(fn_re):
    ...
    sql_tmp = 'select ticker,tradeDate,closePrice as `close` from forex_day where tradeDate>="2014-01-01"'
    x = get_db_data('polygon',sql_tmp)
...
fig=tmp.get_figure()
fig.savefig(os.path.join('Figure','%s2.png' % sub_index_id),bbox_inches='tight',dpi=300)
