...
df = get_MktEqudAdjAfGet_update('000001',start_time,end_time,'tradeDate,openPrice as openIndex,closePrice as closeIndex')
...
cond5_r = df['DIF'] < df['last_valley_dif']  # 条件4：当前DEA比上一次拐点低（DEA创新低）

df.loc[cond2 & cond4 & cond5, 'state'] = '底背离'
# macd柱状图在两次新低之间没有大于0
pos1 = df[df.price_new_low==1]
pos1 = pos1.index.tolist()
pos1.sort()
df['macd_sig'] = True
'''
for sub_p1,sub_p2 in zip(pos1[:-1],pos1[1:]):
    tmp = df.iloc[sub_p1:sub_p2]
    tmp = (tmp.MACD<=0).all()
    df.iloc[sub_p1,df.columns.tolist().index('macd_sig')] = tmp
'''
cond_buy1 = df.state =='底背离'
df.loc[cond_buy1 & df.macd_sig, 'signal'] = 1  # 买入信号（底背离）
#平仓条件
calculate_bollinger_bands(df, column_name = 'close')
df['sell_sig1'] = df.close>=df.RollingMean
df['sell_sig2'] = df.close>=df.UpperBound
#止损条件 创新低止损
df.loc[cond2 & cond4 & cond5_r,'sell_sig3'] = True
