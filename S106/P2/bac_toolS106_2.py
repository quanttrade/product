...
from S106_weight_tool import MVSKT,MVSK
...
r1 = []
for t1,t2 in tqdm(zip(t_test[12:-1],t_test[13:])):
    sub_pool = pool[pool.tradingdate==t1]
    sub_pool = sub_pool.symbol.unique().tolist()
    #计算权重
    #1 定位
    t1_begin = '%d%s' % (int(t1[:4])-1,t1[4:])
    sub_x = x.loc[t1_begin:t1,sub_pool].copy()
    sub_x = sub_x.iloc[1:]
...
r2 = r1.copy()
r2.MVSK = r.r0 - r2.MVSK
r2.MVSKT = r.r0 - r2.MVSKT
