...
if __name__ == "__main__":
    obj_t.use('更新S109P1因子')
    t_fac = get_inidata(tn_fac,eg = eg37)
    t0='2007-01-01'
    tt = '3099-02-20'
    ...
    tt_f = all_factor_df.tradeDate.max()
    #因子中性化
    sdate_backtest = get_delta_date(t_fac,-1)
    edate_backtest = tt_f
    ...
    if len(month_end_list) > 0:
        # 股票池筛选(剔除上市不满60个交易日的次新股、st股、停牌个股、一字板个股)
        ...
        F.to_sql(tn_fac,eg37,index=False,if_exists='append',chunksize=10000,dtype=d)
    obj_t.use('com')