...
save_folder = "S109_P2_data"  # 建立文件夹保存数据量大的数据文件
...
# 全A投资域
a_universe_list = get_symbol_A()


# 个股日度行情数据
fn_data1 = os.path.join(save_folder, 'dmkt.pkl')
if not os.path.exists(fn_data1):
    ...
    dmkt_df.to_pickle(fn_data1)
else:
    dmkt_df = pd.read_pickle(fn_data1)
print("个股日度行情数据:", dmkt_df.head())

# 获取个股月度收益率
mret_df = get_MktEqumAdjGet_update(sdate_backtest, edate_backtest)
...
fn_data3 = os.path.join(save_folder, 'all_factor_df.pkl')
if not os.path.exists(fn_data3):
    # 因子1：revs_20: 传统反转因子，过去20个交易日的close2close收益率累乘
    ...
    all_factor_df.to_pickle(fn_data3)
else:
    all_factor_df = pd.read_pickle(fn_data3)
print("全部反转因子数据:", all_factor_df.tail())
...
fn_data4 = os.path.join(save_folder,'pa_factor_df.pkl')
if os.path.exists(fn_data4):
    pa_factor_df = pd.read_pickle(fn_data4)
else:
    pa_factor_df = factor_process(month_factor_df, factor_list, exclude_style_list=exclude_style_list)
    pa_factor_df.to_pickle(fn_data4)
...
print('corr_overnight_20分年度表现', corr_year_perf.head())
...
factor_list = ['revs_20', 'revs_overnight_20', 'revs_intraday_20', 'abs_overnight_20', 'corr_overnight_20']
exclude_style_list=[]
fn_data5 = os.path.join(save_folder,'a_factor_df.pkl')
if os.path.exists(fn_data5):
    a_factor_df = pd.read_pickle(fn_data5)
else:
    a_factor_df = factor_process(month_factor_df, factor_list, exclude_style_list=exclude_style_list)
    a_factor_df.to_pickle(fn_data5)
...
print('涨跌幅+信息优势双分组测试下月平均超额收益', stat2)
...
fn_data6 = os.path.join(save_folder, 'sensitivity_test_factors_df.pkl')
if not os.path.exists(fn_data6):
    # 停牌时，开盘价处理
    ...
    all_factor_df.to_pickle(fn_data6)
else:
    all_factor_df = pd.read_pickle(fn_data6)
print("不同参数的因子数据:", all_factor_df.tail())



start_time = time.time()
#"对全中性化因子进行IC测试分析和分组测试分析"

# 月末因子值
month_factor_df = all_factor_df[all_factor_df['tradeDate'].isin(month_end_list)]
# 因子方向调整
adj_factor_list = ['revs_5', 'revs_overnight_5', 'abs_overnight_5', 'corr_overnight_5',
               'revs_10', 'revs_overnight_10', 'abs_overnight_10', 'corr_overnight_10',
               'revs_20', 'revs_overnight_20', 'abs_overnight_20', 'corr_overnight_20',
               'revs_40', 'abs_overnight_40', 'corr_overnight_40',
                   'revs_60', 'abs_overnight_60', 'corr_overnight_60']
month_factor_df[adj_factor_list] = -month_factor_df[adj_factor_list]
print(','.join(adj_factor_list) + '为负向因子，进行方向调整')

# # # 行业市值中性化
factor_list = ['revs_5', 'revs_overnight_5', 'abs_overnight_5', 'corr_overnight_5',
               'revs_10', 'revs_overnight_10', 'abs_overnight_10', 'corr_overnight_10',
               'revs_20', 'revs_overnight_20', 'abs_overnight_20', 'corr_overnight_20',
               'revs_40', 'revs_overnight_40', 'abs_overnight_40', 'corr_overnight_40',
               'revs_60', 'revs_overnight_60', 'abs_overnight_60', 'corr_overnight_60']
exclude_style_list=[]
fn_data7 = os.path.join(save_folder, 'fn_data7.pkl')
if not os.path.exists(fn_data7):
    a_factor_df = pd.read_pickle(fn_data7)
else:
    a_factor_df = factor_process(month_factor_df, factor_list, exclude_style_list=exclude_style_list)
    a_factor_df.to_pickle(fn_data7)
...
f0['pre'] = 0
fn_data4 = os.path.join(save_folder,'pa_factor_df.pkl')
f1 = pa_factor_df = pd.read_pickle(fn_data4)
f1['pre'] = 1

fn_data5 = os.path.join(save_folder,'a_factor_df.pkl')
f2 = pd.read_pickle(fn_data5)
f2['pre'] = 2
...
F = F.merge(forbidden_pool,how='left',on=['ticker','tradeDate'])
F = F[F.special_flag.isnull()]
F = F[cols]
