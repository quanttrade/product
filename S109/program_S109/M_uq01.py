...
1.1 获取历年业绩预告数据并进行数量、覆盖度的分析
公司在业绩预告中会公布多类预告值，会有不同的口径，如净利润/营收绝对值，利润/营收相对值，
或者EPS值等，本部分利用API获取所有的预告数据，而在本文后面会将所有同归母净利润相关的值进行换算、
对齐，转换成归母净利润的绝对数值以便于因子计算
'''

ef_df = FdmtEfNewGet('2007-03-31','2023-03-31','*','endDate')
...
for year in range(2007, 2023):
    ...
    fore_df = DataAPI.FdmtEfNewGet(reportType=u"Q1,S1,Q3,CQ3,Q2,A",endDate=enddate_end,
                                   beginDate=enddate_start,pandas="1")
...
# 统计各个财报期的预告数量
print(ef_df.groupby(['reportType'])['secID'].count().reset_index())
...
for univ_name in univ_dict.keys():
    ...
    for tyear in range(2007, 2023):
        sql_tmp = 'select consTickerSymbol from idxconsgets57 where intoDate = "%d-01-01" '
        univ_tmp_df = get_IdxCons('%d-01-01' % tyear,univ_code)
        univ_tmp_df = pd.DataFrame({'consTickerSymbol':univ_tmp_df})
...
datetime_2_str(profit_ef_df , ['endDate', 'publishDate'])
# 业绩预告数据处理，把eps预告、预告增幅等都统一换算为绝对值
stime = time.time()
print("开始取公司的历年财报数据...")
## 获取公司的财报数据
fin_rpt_df = FdmtISGet('2007-01-01','2023-12-31','*',key_words = 'endDate')
...
share_df = EquSharesChgGet('2007-01-01','2023-12-31','ticker,changeDate,totalShares')
...
datetime_2_str(mret_df, 'endDate')
...
pa_factor_df.to_pickle(fn)
pa_factor_df1 = pa_factor_df.copy()
# 因子相关性分析
...
exclude_style_list=['BETA', 'MOMENTUM', 'EARNYILD', 'RESVOL', 'GROWTH', 'BTOP', 'LEVERAGE', 'LIQUIDTY', 'SIZENL']
pa_factor_df = factor_process(month_factor_df, factor_list, exclude_style_list=exclude_style_list)
fn = os.path.join(save_folder,'pa_factor_df2.pkl')
pa_factor_df.to_pickle(fn)
pa_factor_df2 = pa_factor_df.copy()
...
pa_factor_df.to_pickle(fn)
pa_factor_df3 = pa_factor_df.copy()
