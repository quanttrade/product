...
class GlobalLSTMLinear:
    ...
    def __init__(self, x_tasks, model_config):
        ...
        self.transfer_layers = model_config["global_lstm_linear"]["nlayers"]
        ...
        for tk in self.mtl_list:
            ...
            for sub_tk in self.sub_mtl_list[tk]:
                ...
                print(tk, sub_tk, self.model_in_dict[tk][sub_tk], self.model_out_dict[tk][sub_tk],
                      self.global_transfer_lstm, self.signal_layer[tk][sub_tk], self.opt_dict[tk][sub_tk])
        ...
        for i in range(self.tsteps):
            for tk in self.mtl_list:
                for sub_tk in self.sub_mtl_list[tk]:
                    ...
                    (hidden, cell) = self.get_hidden(self.batch_size, self.transfer_layers, self.in_transfer_dim)
        ...
        if self.export_weights:
            for tk in self.mtl_list:
                ...
                torch.save(self.global_transfer_lstm, self.export_path + tk + "_" +
                           self.export_label + "_globaltransferlstm.pt")
        ...
        for tk in self.mtl_list:
            ...
            for sub_tk in self.sub_mtl_list[tk]:
                 ...
                 with torch.autograd.no_grad():
                     ...
                     (hidden, cell) = self.get_hidden(1, self.transfer_layers, self.in_transfer_dim)
                     global_pred, _ = self.global_transfer_lstm(in_pred, (hidden, cell))
