...
from yq_toolsS45_linux import get_us_daytick_adj
...
#import math
from yq_toolsS45_linux import save_pickle
...
def back_test_v2(df, e_list, hold_days = 5, is_plot = True):
    ...
    #第一个信号执行的时候，建仓使用chg0，平仓使用chg1
    #开
    df.loc[(df.sig.shift(1)==1)&(df.sig.shift(2)==0),'r'] = df.loc[(df.sig.shift(1)==1)&(df.sig.shift(2)==0),'chg0']
    #平
    df.loc[(df.sig.shift(1)==0)&(df.sig.shift(2)==1),'r'] = df.loc[(df.sig.shift(1)==0)&(df.sig.shift(2)==1),'chg1']
...
def get_single_s43_v2(x):
    ...
    df.chgPct.fillna(0,inplace=True)
    df.chg0.fillna(0,inplace=True)
    df.chg1.fillna(0,inplace=True)
...
def create_s43_tb():
    ...
    create_table(db_name,tn,var_name,var_type,key_str,1) 
...
def create_s43_tb_v2():
    ...
    create_table(db_name,tn,var_name,var_type,key_str)     
...
def get_ticker_pool(dtype):
    if dtype=="csi":
        ...
        hs_300_pool = get_db_data('data_pro',hs_300_pool)
        ...
        hs_300_pool = get_db_data('data_pro',hs_300_pool % (tn_tdx,dtype,index_tdx[dtype]))
        hs_300_pool = hs_300_pool.ticker.tolist()    
