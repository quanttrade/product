...
def mIdxCloseWeightGet(ticker,begin,end,key_str):
    ...
    x = pd.read_sql(sql_tmp % (key_str,ticker,begin,end),engine)
    return x
...
def get_week_month_tradeDate(start_date,end_date,exchangeCD='XSHG'):
    ...
    calendar_df = pd.read_sql(sql_str_calender % (exchangeCD,start_date,end_date),engine)
...
def MktStockFactorsOneDayProGet_adair(t,field='*'):
    ...
    x = pd.read_sql(sql_tmp % (field,t),engine)
...
def TradeCalGet(t0='1900-01-01',tt='2099-01-01',key_str='calendarDate',exchangeCD='"XSHG"'):
    ...
    t = pd.read_sql(sql_tmp %(key_str,t0,tt,exchangeCD),engine)
    return t
...
def DataAPI_EcoDataProGet(ticker,t0,tt,field='*',asc=False):
    ...
    x = pd.read_sql(sql_str_index,engine)
...
def RMExposureDayGet(beginDate, endDate,field='*',ticker=""):
    ...
    return pd.read_sql(sql_tmp,engine)
...
def get_us_daytick(ticker,t0,tt):
    ...
    x = pd.read_sql(sql_str % (ticker,t0,tt),engine_db_polygon)
    return x
...
def get_forex_mintick(ticker,t0='1990-01-01',tt='2099-01-01'):
    import sqlite3
    with open('para.json','r',encoding='utf-8') as f:
        para = json.load(f)
    p1 = para['sql3_forex']
...
def get_us_adj(ticker):
    ...
    x=pd.read_sql(sql_str % ticker,engine_db_polygon)
    return x
...
def get_spx_com(t0,dis=True):
    x = pd.read_sql('select tradeDate,tickers from sp500_com where tradeDate<="%s" order by tradeDate desc limit 1' % t0,eg_datapro)
...
def get_spx_com_all(t0,tt,dis=True):
    x = pd.read_sql('select tradeDate,tickers from sp500_com where tradeDate>="%s" and tradeDate<="%s" ' % (t0,tt),eg_datapro)
...
def table_in_database(dn,tn):
    x = pd.read_sql('show tables from %s' % dn,engine)
    if len(x)>0:
...
def export_zx02_symbolpool(taskID = 57):
    ...
    def un_fold_data(v):
        ...
        return tmp
    x = pd.read_sql('select * from %s order by tradeDate' % tn,eg_zx2)
...
def yq_MktStockFactorsOneDayProGet_seri(ticker='000001',t0='1990-01-01',tt='3099-01-01',var="*"):
    ...
    x = pd.read_sql(sql_str % (var,ticker,t0,tt),engine)
    return x