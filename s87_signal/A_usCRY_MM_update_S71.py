...
'ZARPLN' 无法写入
最终日期不可以写的太离谱，否者会报错
x=get_aggregates_tick('X:%sUSD' % 'BTC', '2010-01-01','2033-03-27', multiplier=1, 
                              timespan='minute',client=client,unadjusted=True,P0=5000,cry=True)
只针对S71的35只来执行，目的是快速
...
t_end = datetime.datetime.strftime(datetime.datetime.now()+datetime.timedelta(days=20),'%Y-%m-%d')
num_core=32
obj_t = time_use_tool()
...
def MM_flow2(ticker):
    ...
    try:
        ...
        x=get_aggregates_tick('X:%sUSD' % ticker, t0_0,t_end, multiplier=1, 
                              timespan='minute',client=client,unadjusted=True,P0=40000,cry=True)
        client.close()
        #save
        if len(x)>0:
            x = x[x.tradeDate>t0]
...
if __name__ == "__main__":
    ...
    ticker = 'BTC,LTC,ETH,ETC,ZEC,XMR,DASH,XRP'.split(',')
    
    para_fn = os.path.join(r's47_para','cry_t0.pkl')
    if os.path.exists(para_fn):
        t0_com = read_pickle(para_fn)
    else:
        t0_com =pd.DataFrame(index=ticker)
        t0_com['t0'] = '2010-01-01'
